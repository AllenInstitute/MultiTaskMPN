...
def delaygo_(config, mode, anti_response, fix, separate_input, label_strength, long_delay, **kwargs):
    '''
    ...
    if mode == 'random': # Randomly generate parameters, but uniform times/modalities across batch
        ...
        tdim     = fix_offs + stim_offs + (rng.uniform(300,700, size=(batch_size,))/dt).astype(np.int32)
...
def delaygo(config, mode, fix, separate_input, label_strength, long_delay, **kwargs):
    return delaygo_(config, mode, False, fix, separate_input, label_strength, long_delay, **kwargs)

def delayanti(config, mode, fix, separate_input, label_strength, long_delay, **kwargs):
    return delaygo_(config, mode, True, fix, separate_input, label_strength, long_delay, **kwargs)
...
def generate_trials(rule, hp, mode, noise_on=True, fix=False, separate_input=False, label_strength=False, \
    ...
    """
    config = hp
    trial = rule_mapping[rule](config, mode, fix, separate_input, label_strength, long_delay, **kwargs)
