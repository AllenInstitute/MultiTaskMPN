########################################################
# Test for MPN (DMPN) testing on different set of tasks
# 2025-10-30: in the current setting, for which in the 
# modulation layer, pre = post, then the total number of 
# trainable parameter is equivalent to the RNN with same 
# scale (except additional eta/lambda if are trainable)
# as well
########################################################
...
for _ in range(5): 
    ...
    hyp_dict['ruleset'] = 'delaydmfamily' # low_dim, all, test
    ...
    out_path = Path(f"./flextask/param_{hyp_dict['ruleset']}_seed{seed}_{hyp_dict['addon_name']}_param.json")
    ...
    if train:
        ...
        plt.savefig(f"./flextask/loss_{hyp_dict['ruleset']}_seed{seed}_{hyp_dict['addon_name']}.png", dpi=100)
    ...
    def plot_input_output(test_input_np, net_out, test_output_np, test_task=None, tag="", batch_num=5):
        ...
        fig_all.savefig(f"./flextask/lowD_{hyp_dict['ruleset']}_{hyp_dict['chosen_network']}_seed{seed}_{hyp_dict['addon_name']}_{tag}.png", dpi=100)
    ...
    loss_name = f"./flextask/loss_{hyp_dict['ruleset']}_{hyp_dict['chosen_network']}_seed{seed}_{hyp_dict['addon_name']}.pkl"
