...
for _ in range(5): 
    ...
    out_path = Path(f"./onetask/param_{hyp_dict['ruleset']}_seed{seed}_{hyp_dict['addon_name']}_param.json")
    ...
    if train:
        ...
        plt.savefig(f"./onetask/loss_{hyp_dict['ruleset']}_seed{seed}_{hyp_dict['addon_name']}.png", dpi=100)
    ...
    def plot_input_output(test_input_np, net_out, test_output_np, test_task=None, tag="", batch_num=5):
        ...
        fig_all.savefig(f"./onetask/lowD_{hyp_dict['ruleset']}_{hyp_dict['chosen_network']}_seed{seed}_{hyp_dict['addon_name']}_{tag}.png", dpi=100)
