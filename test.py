...
def current_basic_params():
    ...
    train_params = {
        ...
        'n_epochs_per_set': 750, # longer/shorter training
...
# MODIFY BY ZIHAN
# choose the period that is not necessary (reversely counting the mask)
# first_periods = []
test_mask_np = test_mask.detach().cpu().numpy()

# # manual searching
# for batch_iter in range(test_output_np.shape[0]):
#     output_per_batch = test_output_np[batch_iter,:,:]
#     input_per_batch = test_input_np[batch_iter,:,:]
#     mask_per_batch = test_mask_np[batch_iter,:,:]
    
#     zero_lists = helper.find_consecutive_zero_indices(mask_per_batch)
#     subfirst_periods = []
#     for jj in range(len(zero_lists)):
#         subfirst_periods.append(zero_lists[jj][0])
#     first_periods.append(subfirst_periods)
    ...
    for pca_target in pca_target_lst:
        ...
        if session_iter == len(session_breakdown)-1 and pca_target in ('hs', 'Ms'):
            ...
            for ii in range(len(session_breakdown[:-1])):
                ...
                all_normal_vectors.append(normal_vector)
            ...
            # what if we use all PCs

            # if pca_target in ('hs',):
            #     W_output_pca_response = W_output_pca[1:,:] - zeros_pca
            #     responserecord_all = []

            #     for batchiter in range(as_pca.shape[0]):
            #         responserecord = []
            #         for timeiter in range(as_pca.shape[1]):
            #             time_spec = as_pca[batchiter, timeiter, :].reshape(1,-1)

            #             results = cca_core.get_cca_similarity(time_spec, W_output_pca_response, epsilon=1e-10, verbose=False)
            #             val = np.mean(results["cca_coef1"])
            #             responserecord.append(val)
            #         responserecord_all.append(responserecord)
                
            #     responserecord_all = np.array(responserecord_all)
            #     response_similarity_mean, response_similarity_std = np.mean(responserecord_all, axis=0), np.std(responserecord_all, axis=0)
                
            #     figsvcca, axssvcca = plt.subplots()
            #     axssvcca.plot(xxx, response_similarity_mean)
            #     axssvcca.fill_between(xxx, response_similarity_mean-response_similarity_std, response_similarity_mean+response_similarity_std, alpha=0.3)
            #     figsvcca.savefig(f"./results/zz_test_{pca_target}_{ruleset}_{chosen_network}_{addon_name}_svcca_response.png")
